...
export function PaymentIcon({ type, brand, size = "md", className = "" }: PaymentIconProps) {
  ...
  if (type === "card") {
    return (
      <div className={`${containerSizes[size]} bg-gradient-to-r from-slate-600 to-slate-800 rounded-lg flex items-center justify-center shadow-sm ${className}`}>
  ...
  if (type in WALLET_CONFIG) {
    ...
    return (
      <div className={`${containerSizes[size]} ${config.container} rounded-lg flex items-center justify-center shadow-sm ${className}`}>
        <img 
          src={config.icon}
          alt={type.replace('_', ' ')}
...
export function WalletIcon({ wallet, className = "" }: { wallet: string; className?: string }) {
  const icons: Record<string, JSX.Element> = {
    ...
    apple_pay: (
      <div className={`inline-flex items-center justify-center w-6 h-6 bg-black rounded ${className}`}>
        <img 
  ...
  return icons[wallet] ?? (
    <div className={`inline-flex items-center justify-center w-6 h-6 bg-gray-500 rounded ${className}`}>
